...
const AdminLogin = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Using FormData for sending the form data securely
    ...
    try {
      ...
      if (data.success) {
        ...
        setTimeout(() => {
          // Save user data or token to localStorage for future requests
          localStorage.setItem('adminEmail', data.email);
          navigate(`/admin-dashboard?email=${data.email}`); // Pass email via query params or use state
        }, 2000);
      } else {
